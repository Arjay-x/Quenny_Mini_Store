...
$items_array = [];
$items_result = $conn->query("SELECT i.*, c.category_name FROM items i LEFT JOIN categories c ON i.category_id = c.category_id ORDER BY i.item_name ASC");
if ($items_result) {
    while ($item = $items_result->fetch_assoc()) {
        $cat_name = strtolower(str_replace(' ', '', $item['category_name'] ?? 'others'));
